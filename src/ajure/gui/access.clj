...
(defn add-combo-to-map [combo-map-ref modifiers key action]
  (dosync
...
(defmacro def-editor-combo [modifiers key & body]
  `(add-combo-to-map editor-key-combos
                     (vector ~@(map (fn [x] (symbol "org.eclipse.swt.SWT" (str x)))
                                    modifiers))
                     ~key
                     (fn []
                       ~@body)))
...
(defmacro def-app-combo [modifiers key & body]
  `(add-combo-to-map application-key-combos
                     (vector ~@(map (fn [x] (symbol "org.eclipse.swt.SWT" (str x)))
                                    modifiers))
                     ~key
                     (fn []
